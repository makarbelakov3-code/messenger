...
function loadUsers() { try { if (fs.existsSync(USERS_FILE)) return JSON.parse(fs.readFileSync(USERS_FILE, 'utf8')); } catch(e) {} return {}; }
function loadChats() { try { if (fs.existsSync(CHATS_FILE)) return JSON.parse(fs.readFileSync(CHATS_FILE, 'utf8')); } catch(e) {} return {}; }
...
app.post('/api/login', (req, res) => {
    ...
    if (!user || !bcrypt.compareSync(password, user.password)) return res.status(401).json({ error: 'Неверное имя или пароль' });
...
app.get('/api/chats/:userId', (req, res) => {
    ...
    for (const [id, chat] of Object.entries(chats)) {
        if (chat.participants.includes(req.params.userId) || chat.type === 'public') {
            ...
            result.push({ id, type: chat.type, name: displayName, lastMessage: lastMsg, updatedAt: lastMsg?.createdAt || chat.createdAt });
        }
    }
...
io.on('connection', (socket) => {
    ...
    socket.on('create_private_chat', ({ targetUserId }) => {
        ...
        if (existing) { socket.emit('chat_created', { chatId: existing.id, chat: existing }); return; }
    ...
    socket.on('send_message', ({ chatId, text, files, replyTo }) => {
        if (!currentUserId) return;
    ...
    socket.on('react_to_message', ({ chatId, messageId, emoji }) => {
        ...
        if (msg) {
            ...
            if (msg.reactions[currentUserId] === emoji) delete msg.reactions[currentUserId];
            else msg.reactions[currentUserId] = emoji;
            const chat = chats[chatId];
            chat.participants.forEach(pid => {
                const s = userSockets.get(pid);
    ...
    socket.on('update_profile', ({ name, bio, avatar }) => {
        if (!currentUserId) return;
        const user = Object.values(users).find(u => u.id === currentUserId);
        if (user) {
            ...
            saveUsers();
            io.emit('user_updated', { userId: user.id, name: user.name, avatar: user.avatar });
    ...
    socket.on('call_user', ({ targetUserId, offer }) => {
        const targetSocket = userSockets.get(targetUserId);
        if (targetSocket) io.to(targetSocket).emit('incoming_call', { from: currentUserId, offer });
    });
